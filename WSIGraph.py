...
def constructGraphFromDict(
    ...
    # nucleusInfo['nuc'].values() 直接去掉了keys()的信息，保留了values()（值）集合，无编号，所以下面定义name来作为图中节点编号
    for nucInfo in tqdm(nucleusInfo['nuc'].values(),desc="0. Preparing"):
    ...
    assert len(bboxes) == len(centroids) == len(types), 'The attribute of nodes (bboxes, centroids, types) must have same length'
    ...
    nolabeIDs, neoplaIDs, inflamIDs, connecIDs, necrosIDs, normalIDs = \
        [np.where(np.array(types) == i)[0].tolist() for i in range(6)] #这些index是根据types筛选的，编号与names一致
    # timeMorph, timeGLCM = 0, 0
    print(len(neoplaIDs), len(inflamIDs), len(connecIDs))
    ...
    for featype, featype_index_list in zip(featype_dict.keys(),
        ...
        if len(featype_index_list[0])==0 or len(featype_index_list[1])==0:
            print(len(featype_index_list[0]), len(featype_index_list[1]),"with zero")
            continue
        ...
        subgraph_names = []
        featype_index = []#节点的index，与neoplaIDs, inflamIDs, connecIDs一致
